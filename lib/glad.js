...
import { ensureEndsWith, ensureNotEndingWith } from './util/strings.js'
...
export class GLAD {
    ...
    generateDependenciesFromSourceFiles () {
        ...
        if (startingPath && startingPath !== '.') {
            ...
            startingPath = '.' + path.sep
        }
        startingPath = ensureEndsWith(startingPath, path.sep)
    ...
    processFileAsContainer (pathToFile) {
        const listOfContainer = pathToFile.split(path.sep).filter(token => token !== '')
