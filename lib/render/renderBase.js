...
import { plural, XMLTree, stringIsEmpty } from '../util/strings.js'
import { Edges } from '../models/edges.js'
import { Container } from '../models/container.js'
import { Layer } from '../models/layer.js'
import { getLayersInThisNode, Layers } from '../models/layers.js'
import { Point, Rectangle } from '../models/rectangle.js'
...
export class RenderBase {
  ...
  renderLabels (node) {
    ...
    if (node.isLeaf) {
      ...
      if (node.versionToString() !== '') {
        this.svgDoc += `<text class="nodeName" x="${r.centerHorizontal}" y="${r.centerVertical + 20}" font-size="80%">${node.versionToString()}</text>`
      }
    } else {
    ...
    if (this.options.details) {
      ...
      if (!node.isLeaf) {
        ...
        if (!stringIsEmpty(pathToThisContainer) && pathToThisContainer !== '.') {
          itemsToShow.push(`path: .../${pathToThisContainer}/`)
        }
      }

      const textInfo = itemsToShow.join(', ')

      if (!stringIsEmpty(textInfo)) {
        this.svgDoc += `<text x="${r.x + Constants.paddingText}" y="${r.bottom - Constants.paddingText}" fill='black' font-size="75%">`
    ...
    if (node.isLeaf) {
      if (this.options.lines !== Constants.LINES_HIDE) {
        if (node.targetNodes.length > 0) {
          const tooltip = nodeName + '\n⬇\n' + Container.getNodeListAsToolTip(node.targetNodes)
          this.addPillCounter(getEdgeDeparturePoint(node), node.targetNodes.length.toString(), 'green', 'white', tooltip)
        }

        if (node.sourceNodes.length > 0) {
          const tooltip = Container.getNodeListAsToolTip(node.sourceNodes) + '\n⬇\n' + nodeName
          this.addPillCounter(getEdgeArrivalPoint(node), node.sourceNodes.length.toString(), 'blue', 'white', tooltip)
        }
      }
    }
   ...
   */
  addPillCounter (centerPoint, value, backgroundColor = '#000011aa', fontColor = 'white', tooltip = '') {
    this.addPillCounterXY(centerPoint.x, centerPoint.y, value, backgroundColor, fontColor, tooltip)
  }
   ...
   * @param {string} tooltip
   */
  addPillCounterXY (x, y, value, backgroundColor = '#000011aa', fontColor = 'white', tooltip = '') {
    ...
    this.svgDoc += value
    this.svgDoc += '</text>'
    if (!stringIsEmpty(tooltip)) {
      this.svgDoc += '<title>' + tooltip + '</title>'
    }
    this.renderSvgGroupEnd()
  }
  ...
  renderEdgesOfLeaves (edges) {
    ...
    edges.forEach(edge => {
      ...
      const tooltip = edge.source.name + '\n⬇\n' + edge.target.name
      this.addLinePointer(sourceCenter, targetCenter, edge.isCircular, tooltip)
    })
    this.renderSvgGroupEnd()
  }
  ...
  renderEdgesOfContainer (containers) {
    ...
    containerEdgeDeparture.forEach((valueSet, keySource) => {
      valueSet.forEach(target => {
        ...
        this.addPillCounter(sourceCenter, containerCounterForSources[keySource.id], 'green', 'white', '123')
        this.addPillCounter(targetCenter, containerCounterForTargets[target.id], 'blue', 'white', '321')
      })
    })
   ...
   * @param {string} tooltip
   */
  addLinePointer (pointSource, pointTarget, isCircular = false, tooltip = '') {
    ...
    if (isCircular) {
      ...
      if (y1 < y2) {
        ...
        color = '#377E22' // green & transparent
      } else {
        color = '#F09235' // orange
      }
    }
    ...
    switch (this.options.lines) {
      case Constants.LINES_STRAIT:
        this.svgDoc += getSvgLine(x1, y1, x2, y2, color, shadow, tooltip)
        break
      case Constants.LINES_ELBOW:
        this.svgDoc += getSvgConnectionLine(x1, y1, x2, y2, color, shadow, tooltip)
        break
      case Constants.LINES_ANGLE:
        this.svgDoc += getSvgConnectionAngle(x1, y1, x2, y2, color, shadow, tooltip)
        break
      case Constants.LINES_CURVE:
      default:
        this.svgDoc += getSvgCurve(x1, y1, x2, y2, color, shadow, tooltip)
        break
    }
  }
  ...
  renderSvgStart (w = 1000, h = 1000) {
    ...
    this.svgDoc += `<svg width="${w}" height="${h}" xmlns="http://www.w3.org/2000/svg" font-family="Arial, Helvetica, sans-serif">`
...
`<style>
    ...
    this.svgDoc += '<linearGradient id="layers" gradientTransform="rotate(90)"><stop offset="0%" stop-color="#691872" stop-opacity="0.1"/><stop offset="100%" stop-color="#691872" stop-opacity="0.3" /></linearGradient>'
    ...
    this.svgDoc += '<path d=\'M0,-16L32,0L0,16\' fill=\'#3058FF\' opacity="0.67"/>'
    this.svgDoc += '</marker>'

    this.svgDoc += '<marker id=\'lineStart\' viewBox= \'0 -4 8 8\' refX=\'4\' orient=\'auto\'>'
    this.svgDoc += '<circle cx=\'4\' r=\'4\' fill=\'#377E22\'/>'
    this.svgDoc += '</marker>'
