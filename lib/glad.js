...
export class GLAD {
  ...
  loadGraphFromJSON (jsonFile) {
    ...
    blob.packages.forEach(module => {
      this.upsertInContainer(module.source, module.name)
    })

    // Add all the eddges
    blob.packages.forEach(module => {
      module.dependencies.forEach(depName => {
        this.ensureContainmentAndLinkTheseTwo(module.source, module.name, depName)
      })
    })
  ...
  ensureContainmentAndLinkTheseTwo (containerName, sourceName, targeName) {
    // printAction('-------- ' + containerName + ' ' + sourceName + ' ' + targeName)

    this.upsertInContainer(containerName, sourceName)
    this.graph.upsertFileLinkByText(sourceName, targeName)
  }
  ...
  upsertInContainer (containerName, nodeName) {
    ...
    if (containerFound) {
      containerFound.upsert(nodeName).setAsLeaf(nodeName)
    }
  }
}
